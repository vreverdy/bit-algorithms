...
auto BM_BitShiftRight = [](benchmark::State& state, auto input) {
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(bit::shift_right(first, last, distr(generator)));
...
auto BM_BitShiftRight_UU = [](benchmark::State& state, auto input) {
    ...
    auto last = bit::bit_iterator<decltype(std::end(bitcont))>(std::end(bitcont));
    for (auto _ : state) {
        benchmark::DoNotOptimize(bit::shift_right(first + 2, last - 3, distr(generator)));
        benchmark::ClobberMemory();
    }
};
...
auto BM_BoolShiftRight = [](benchmark::State& state, auto input) {
    ...
    std::uniform_int_distribution<int>  distr(range_from, range_to);
    for (auto _ : state) {
        benchmark::DoNotOptimize(bit::word_shift_right(first, last, distr(generator)));
        benchmark::ClobberMemory();
    }
};
